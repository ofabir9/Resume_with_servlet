...
//        	out.write("\nDebug project\n "+achievementNames[i]+"\n");
        ...
        Employee oldEmployee = EmployeeService.getEmployeeService().getEmployeeById(Integer.parseInt(request.getParameter("oldEmployeeId")));
        if(errors.isEmpty())
        {
        	
        	
        	EmployeeService.getEmployeeService().updateEmployee(oldEmployee,employee);
            response.sendRedirect("ResumeServlet?id="+employee.getId());
	    }
        else
        {
